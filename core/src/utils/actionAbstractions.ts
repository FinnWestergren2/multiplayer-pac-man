...
export const handlePlayerInput = (store: ReduxStore, playerId: string, stampedInput: StampedInput) => {
    switch(stampedInput.input.type) {
        case InputType.MOVE_UNIT:
            ...
            console.log(`${playerId.substring(0, 4)} made a move input ${stampedInput.timeAgo}ms ago at ${stampedInput.input.origin} \t`, 'distance travelled since then', distTravelled);
